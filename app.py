...
#  URLチェック
def check_url(row_idx, url, kw1_list, kw2_list, use_kw1, use_kw2):
    try:
        if not url:
...
if uploaded_file and st.session_state.run:
    ...
    use_kw1 = len(kw1_list) > 0
    use_kw2 = len(kw2_list) > 0

    if not use_kw1 and not use_kw2:
        st.warning("KWを1つ以上入力してください")
        st.stop()
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            executor.submit(check_url, i, ws[f"G{i}"].value, kw1_list, kw2_list, use_kw1, use_kw2)
